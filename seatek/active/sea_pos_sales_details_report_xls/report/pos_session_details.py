...
class SalesDetailsReportXLS(models.AbstractModel):
    _name = 'report.sea_sales_details_report.pos_session_details'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        worksheet1.merge_range(row_no, 1, row_no, 4,
                               'Từ ngày ' + str(objects.start_at.strftime('%d-%m-%Y')) + ' Đến ngày ' +
                               str(objects.stop_at.strftime('%d-%m-%Y')), f4)
        worksheet1.merge_range(row_no, 5, row_no + 1, 9, 'CHI TIẾT HOẠT ĐỘNG TRONG NGÀY', f5)
        row_no += 1
        worksheet1.merge_range(row_no, 1, row_no, 4, 'Thu ngân: ' + str(objects.user_id.name), f4)
        row_no += 1
        ...
        atm = 0
        voucher = 0
        orders = self.env['pos.order'].search([('session_id', '=', objects.name)])
        for order in reversed(orders):
            ...
            for payment in order.statement_ids:
                if payment.journal_id.id == 127 or payment.journal_id.id == 15 or payment.journal_id.id == 135 \
                    ...
                    cash += payment.amount
                    worksheet1.write(row_no, 6, payment.amount - amount_return or 0, f11)
